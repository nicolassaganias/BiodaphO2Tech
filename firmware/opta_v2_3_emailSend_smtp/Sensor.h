...
float readOxygenSensor() {
  double voltage = 0.0, current = 0.0, oxygen_sensor_value = 0.0;
  for (int i = 0; i < 10; i++) {
    double rawValue = analogRead(SENSOR_DISSOLVE_OXYGEN_PIN);  // Read ADC
...
float readConductivitySensor() {
  double voltage = 0.0, current = 0.0, ec_sensor_value = 0.0;
  for (int i = 0; i < 10; i++) {
    double rawValue = analogRead(SENSOR_CONDUCTIVITY_PIN);  // Read ADC
...
// Read pH sensor
float readpHSensor() {
  double voltage = 0.0, current = 0.0, ph_sensor_value = 0.0;
  for (int i = 0; i < 10; i++) {
    double rawValue = analogRead(SENSOR_PH_PIN);  // Read ADC

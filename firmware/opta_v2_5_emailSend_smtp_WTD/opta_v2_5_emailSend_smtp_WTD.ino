...
void setup() {
  ...
  mbed::Watchdog::get_instance().start(300000);  // Configurar el Watchdog con un tiempo de espera de 5 min
...
void loop() {
  ...
  if (millis() - lastNTPUpdate >= NTP_SYNC_INTERVAL) {
    syncNTP();
    lastNTPUpdate = millis();  // Registrar el último tiempo de actualización
  }
}

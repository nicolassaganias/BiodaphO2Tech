...
float readOxygenSensor() {
  float voltage, current, oxygen_sensor_value = 0;
  for (int i = 0; i < 10; i++) {
    voltage = readOxygenADC();
    current = (voltage / OXYGEN_SENS_RESISTOR_VALUE) * 1000.00;
    oxygen_sensor_value += min(max(0, mapFloat(current, 4.00, 20.00, OXYGEN_SENS_LOWER_VALUE, OXYGEN_SENS_HIGHER_VALUE)), OXYGEN_SENS_HIGHER_VALUE);
    delay(10);
  }
  oxygen_sensor_value /= 10;
  if (oxygen_sensor_value >= OXYGEN_SENS_LOWER_VALUE && oxygen_sensor_value <= OXYGEN_SENS_HIGHER_VALUE) {
    return oxygen_sensor_value;
  } else {
    Serial.println(" Oxyegn Warning: The sensor might be connected incorrectly or receiving inaccurate values.");
    return 0;
  }
}
...
float readConductivitySensor() {
  float voltage, current, ec_sensor_value = 0;
  for (int i = 0; i < 10; i++) {
    voltage = readECADC();
    current = (voltage / EC_SENS_RESISTOR_VALUE) * 1000.00;
    ec_sensor_value += min(max(1, mapFloat(current, 4.00, 20.00, EC_SENS_LOWER_VALUE, EC_MAX_SENSOR_VALUE)), EC_MAX_SENSOR_VALUE);
    delay(10);
  }
  ec_sensor_value /= 10;
  if (ec_sensor_value <= EC_TDS_VALUE_RANGE) {
    ...
    Serial.println(" EC Warning: The sensor might be connected incorrectly or the wrong sensor probe may have been selected.");
    return 0;
  }
}
...
float readpHSensor() {
  float voltage, current, ph_sensor_value = 0;
  for (int i = 0; i < 10; i++) {
    voltage = readpHADC();
    current = (voltage / PH_SENS_RESISTOR_VALUE) * 1000.00;
    ph_sensor_value += min(max(0.0, mapFloat(current, 4.00, 20.00, PH_SENS_LOWER_VALUE, PH_SENS_HIGHER_VALUE)), PH_SENS_HIGHER_VALUE);
    delay(10);
  }
  ph_sensor_value /= 10;
  if (ph_sensor_value >= PH_SENS_LOWER_VALUE && ph_sensor_value <= PH_SENS_HIGHER_VALUE) {
    return ph_sensor_value;
  } else {
    Serial.println(" pH Warning: The sensor might be connected incorrectly or receiving inaccurate values.");
    return 0;
  }
}
...
void healthCheck() {
  ...
  if (WiFi.status() != WL_CONNECTED) errorReport += "- WiFi disconnected\n";
  if (errorReport.length() > timestamp.length() && millis() - lastStatusEmailSent > emailInterval) {
    Serial.println(errorReport);
    lastStatusEmailSent = millis();
  }*/
}